...
def apply_perks(df):
    ...
    for _, row in df.iterrows():
        ...
        elif row['Verizon_Service'] == 'Verizon Plus Play' and row['Customer_Type'] == 'Bad':
            perks.append("Bonus content for the next month")
        else:
...
# Increase days watched for bad Verizon Plus Play subscribers and highlight them
final_df.loc[(final_df['Verizon_Service'] == 'Verizon Plus Play') & (final_df['Customer_Type'] == 'Bad'), 'Days_Watched'] += 5
final_df.loc[(final_df['Verizon_Service'] == 'Verizon Plus Play') & (final_df['Customer_Type'] == 'Bad'), 'Highlighted'] = "Increased Days & Updated Perks"
...
final_df.to_csv("Updatedperks_plusplay.csv", index=False)

# Analyze the changes
bad_to_good = final_df[(final_df['Verizon_Service'] == 'Verizon Plus Play') & (final_df['Customer_Type'] == 'Good')]
print(f"Number of subscribers who moved from Bad to Good: {len(bad_to_good)}")

# Optional: Save a separate CSV for those who moved from Bad to Good
bad_to_good.to_csv("verizon_plusplay_bad_to_good.csv", index=False)
